...
export default function TableItem({ name }) {
    ...
    // Re-inicializa a Preline UI quando as colunas são carregadas
    useEffect(() => {
        if (columns.length > 0) {
            setTimeout(() => {
    ...
    const handleExpand = async (event) => {
        // A CHAVE: Impede que o clique "borbulhe" para os acordeões pais da Preline
    ...
    return (
        // Este componente não é um acordeão da Preline, é só uma div
        <div role="treeitem">
            <div className="py-0.5 flex items-center gap-x-0.5 w-full">
                {/* O botão agora passa o evento para o handler */}
