...
export default function RegisterExerciseLists() {
    ...
    async function handleSubmit (e) {
        ...
        const isAnyExerciseEmpty = exercises.some(ex => !ex.name || !ex.description || !ex.solution_query);
    ...
    return (
        <>
            <div className="relative h-screen flex flex-col items-center bg-gray-900 p-4 sm:p-6 md:p-8">
                ...
                {/* A MUDANÇA ESTÁ AQUI: Aumentamos o z-index para garantir que o botão fique no topo */}
                <div className="absolute top-6 left-6 z-30">
                    <HamburguerMenu onOpen={() => setIsDrawerOpen(true)} />
                </div>
                
                <form key={resetKey} className="w-[60%] h-[95%] bg-[#202024] flex flex-col" onSubmit={handleSubmit}>
        
                    <div className="w-full px-[3%] py-4">
                        <div>
                            <label htmlFor="list_name" className="block text-xs sm:text-sm text-white opacity-30 font-extralight mt-2">
                                Lista de Exercícios
                            </label>
